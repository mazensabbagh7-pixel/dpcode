...
it.layer(
  ...
  it.effect("normalizes stale active turns from ready session shell summaries", () =>
    Effect.gen(function* () {
      const eventStore = yield* OrchestrationEventStore;
      const projectionPipeline = yield* OrchestrationProjectionPipeline;
      const sql = yield* SqlClient.SqlClient;
      const now = "2026-02-27T10:00:00.000Z";
      const threadId = ThreadId.makeUnsafe("thread-ready-stale-active-turn");
      const staleTurnId = TurnId.makeUnsafe("turn-ready-stale-active-turn");

      yield* eventStore.append({
        type: "project.created",
        eventId: EventId.makeUnsafe("evt-ready-stale-project"),
        aggregateKind: "project",
        aggregateId: ProjectId.makeUnsafe("project-ready-stale"),
        occurredAt: now,
        commandId: CommandId.makeUnsafe("cmd-ready-stale-project"),
        causationEventId: null,
        correlationId: CorrelationId.makeUnsafe("cmd-ready-stale-project"),
        metadata: {},
        payload: {
          projectId: ProjectId.makeUnsafe("project-ready-stale"),
          title: "Project",
          workspaceRoot: "/tmp/project-ready-stale",
          defaultModelSelection: null,
          scripts: [],
          createdAt: now,
          updatedAt: now,
      ...
      yield* eventStore.append({
        type: "thread.created",
        eventId: EventId.makeUnsafe("evt-ready-stale-thread"),
        aggregateKind: "thread",
        aggregateId: threadId,
        occurredAt: now,
        commandId: CommandId.makeUnsafe("cmd-ready-stale-thread"),
        causationEventId: null,
        correlationId: CorrelationId.makeUnsafe("cmd-ready-stale-thread"),
        metadata: {},
        payload: {
          threadId,
          projectId: ProjectId.makeUnsafe("project-ready-stale"),
      ...
      yield* eventStore.append({
        type: "thread.session-set",
        eventId: EventId.makeUnsafe("evt-ready-stale-session"),
        aggregateKind: "thread",
        aggregateId: threadId,
        occurredAt: now,
        commandId: CommandId.makeUnsafe("cmd-ready-stale-session"),
        causationEventId: null,
        correlationId: CorrelationId.makeUnsafe("cmd-ready-stale-session"),
        metadata: {},
        payload: {
          threadId,
          session: {
      ...
      const rows = yield* sql<{
        ...
        SELECT
          projection_threads.latest_turn_id AS "latestTurnId",
          projection_thread_sessions.active_turn_id AS "activeTurnId"
        FROM projection_threads
        INNER JOIN projection_thread_sessions
          ON projection_thread_sessions.thread_id = projection_threads.thread_id
        WHERE projection_threads.thread_id = ${threadId}
      `;
      assert.deepEqual(rows, [{ latestTurnId: null, activeTurnId: null }]);
    }),
  );
