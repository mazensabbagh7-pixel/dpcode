...
describe("deriveWorkingTreeDiffSummaryCardState", () => {
  ...
  it("caps retained file rows while preserving aggregate count", () => {
    ...
    const state = deriveWorkingTreeDiffSummaryCardState({
      status: gitStatus({
        hasWorkingTreeChanges: true,
        workingTree: { files, insertions: 25, deletions: 0 },
      }),
      patch: "diff --git",
      summary: null,
    });
    expect(state.filesChanged).toBe(25);
    expect(state.files).toHaveLength(20);
    expect(state.insertions).toBe(25);
  });
});
